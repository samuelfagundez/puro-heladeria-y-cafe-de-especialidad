  ...
  return (
    <section id="galeria" className="bg-cream-dark/50 py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-navy">Galería</p>
          ...
          <div className="flex touch-pan-y">
            {content.gallery.map((photo) => (
              ...
              >
                ...
                {photo.credit && (
                  <p className="mt-1.5 text-right text-xs text-ink/45">Foto: {photo.credit} · Google Maps</p>
                )}
              </div>
            ))}
          </div>
            ...
            {content.gallery.map((photo, i) => (
              <span
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === selected ? 'w-6 bg-navy' : 'w-1.5 bg-ink/20'
                }`}
              />
            ))}
          </div>
